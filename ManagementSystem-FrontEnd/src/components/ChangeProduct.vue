...
export default {
  ...
  methods: {
    ...
    uploadProduct() {
      ...
      var url = this.HOST + "/wx_mini/changeProduct";
      $.post(url,{
  ...
  mounted() {
    ...
    .always(() => {
      ...
      //初始化待修改的产品的信息
    var url = this.HOST + "/wx_mini/getProductById" + "?id=" + this.$route.params.productId;
    $.get(url)
    .then(response => {
      // log("初始化待修改的产品的信息: " + JSON.stringify(response))
