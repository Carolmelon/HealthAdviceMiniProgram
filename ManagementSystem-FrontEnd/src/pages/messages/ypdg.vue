  ...
  <div id="ypdg">
        <div>
            ...
            <div class="table" style="padding: 20px;">
                ...
                <span style="position:absolute;left:10px;font-size:12px;">{{item.name}}</span>
                <div style="position:absolute;left:10%;display:inline-block">
                    <span v-if="item.imageNames == 0">未上传图片</span>
                    <div style="display:inline-block" v-for="img_url in item.imageNames" :key="img_url">
                    <a target="_blank" :href="HOST + img_url">
                        <img :src="HOST + img_url" alt="" height="55px;" width="55px" style="padding:1px;">
                    </a>
                    </div>
                </div>
        ...
        <div id="ypdgDetails" style="position:fixed;left:0;right:0;bottom:0;top:0;background: rgba(0,0,0,0.7);display:none;">
            <!-- 编辑订单信息 -->
            <div
            style="width:800px;min-height:250px;background-color:#fff;margin:150px auto;position:relative;border-radius:20px;overflow:auto;padding-bottom:20px;">
                <el-button @click="switchDetails(null)" style="position:absolute;right:10px;top:10px;" type="primary" plain>关闭</el-button>
                <div style="width:600px;padding:20px;">
                    <div style="height:40px;line-height:40px;border-bottom:1px solid #eee;">药品：{{currentDetail.name}}</div>
                    <div style="height:40px;line-height:40px;border-bottom:1px solid #eee;">用户id：{{currentDetail.openid}}</div>
                    <div style="height:40px;line-height:40px;border-bottom:1px solid #eee;">联系电话：{{currentDetail.tel}}</div>
                    <div style="height:40px;line-height:40px;border-bottom:1px solid #eee;">生产厂家：{{currentDetail.changjia}}</div>
                    <div style="height:40px;line-height:40px;border-bottom:1px solid #eee;">规格：{{currentDetail.guige}}</div>
                    <div style="height:40px;line-height:40px;border-bottom:1px solid #eee;">数量：{{currentDetail.amount}}</div>
                    <div style="height:40px;line-height:40px;border-bottom:1px solid #eee;">所开药品医院：{{currentDetail.hospital}}</div>
