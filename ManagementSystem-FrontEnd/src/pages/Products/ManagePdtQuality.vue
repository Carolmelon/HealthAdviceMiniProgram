  ...
  <div id="ManageArticle">
    ...
    <div class="inner-bottom">
      ...
      <div style="height: 50px; background-color:#aaa;position:relative;padding-top:10px;">
        <b style="position:absolute;left:30px;">商品名称</b>
        <b style="position:absolute;right:60%;">类别</b>
        <b style="position:absolute;right:50%;">排序</b>
        <b style="position:absolute;right:30%;">发布时间</b>
         ...
         style="height:40px;position:relative;padding-top:10px;border-bottom:1.5px solid #ccc">
          ...
          <div style="position:absolute;right:0;">
            <el-button
                type="text"
                size="mini"
                @click="gotoChangeProduct(item.id)">
                编辑
            </el-button>
            <el-button
                type="text"
...
export default {
  ...
  methods: {
    gotoChangeProduct(id){
      log(id)
      this.$router.push({ name: 'ChangeProduct', params: { productId: id }})
    },
    showArticle(id){
      log(id)
      this.$confirm('确认将该产品移出仓库？')
      .then(_ => {
        var url = this.HOST + "/wx_mini/showProduct";
        $.post(url,{
          id: id,
        })
        .then(response => {
          ...
          this.refreshArticleList();  // 刷新一下产品列表
        })
        .catch(err => {
            log(err)
            this.$notify.error({
                title: '错误',
                message: '产品移出仓库失败，请重试或联系管理员'
            })
        })
        done();
      })
    ...
    deleteArticle(id){
      ...
      .then(_ => {
        var url = this.HOST + "/wx_mini/deleteProduct";
        $.post(url,{
          id: id,
        })
        .then(response => {
    ...
    refreshArticleList(){
      ...
      var url = this.HOST + "/wx_mini/getProductListData" + 
    ...
    storeArticleOrder(){
      ...
      var url = this.HOST + "/wx_mini/storeProductOrder";
      $.post(url,{
